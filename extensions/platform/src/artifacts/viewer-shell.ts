/**
 * Markup and CSP strings for the local Artifact viewer, plus the Artifact
 * response CSP that the Vercel adapter reuses.
 *
 * `viewerShell` is the trusted page at `/open`: it strips the Capability
 * Token from the URL fragment before any fetch, exchanges it at `/session`,
 * and frames content with `sandbox="allow-scripts"` (no `allow-same-origin`).
 * `shellCsp` limits the shell to nonce scripts/styles plus Trusted Types.
 * `artifactCsp` always applies a CSP sandbox and `connect-src 'none'`, denies
 * forms, frames, workers, and `<base>`, and allows scripts only when
 * `interactive`.
 *
